...
- [x] Ball
  ...
  - [x] 서로 다른 볼에 대하여 같은 숫자이면 같은 볼이다.
- [x] Balls (볼 1:1 비교 -> 볼 N:1 비교 -> 볼 N:N 비교로 나누어 생각하기)
  - [x] 반드시 3 개의 Ball 을 가져야 한다.
  - [x] 각 볼끼리 숫자가 겹칠 수 없다.
  - [x] 각 볼의 위치와 숫자가 하나 같으면 1스트라이크이다.
  - [x] 각 볼의 숫자 1개가 같지만 위치가 다를 경우 1볼이다.
  - [x] 스트라이크가 2개이면 2스트라이크이다.
  - [x] 볼이 2개이면 2볼이다.
  - [x] 볼이 3개이면 3볼이다.
  - [x] 볼과 스트라이크에 둘다 해당될 경우 해당 볼과 스트라이크를 출력한다.
